...
func (hw HttpWriter) Write(report types.Report, customHeaders http.Header) error {
	...
	if err := hw.publishReport(output, customHeaders); err != nil {
		return xerrors.Errorf("failed to publish report: %w", err)
	}
	return nil
}

func (hw HttpWriter) publishReport(report []byte, customHeaders http.Header) error {
	...
	for {
		...
		// log.Infof("Publishing result to %s (attempt %d)", hw.ListenerUrl, counter+1)
		// log.Infof("printing headers: %v", req.Header)
